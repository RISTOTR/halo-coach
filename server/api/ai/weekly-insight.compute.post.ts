...
export default defineEventHandler(async (event) => {
  ...
  const lastUseByLever = new Map<string, string>()
  ...
  // --- Patterns (cross-experiment “what works”)
  const byRef = await buildLeverSummary({ supabase, uid, windowDays: 180, groupBy: 'lever_ref' })
  const byType = await buildLeverSummary({ supabase, uid, windowDays: 180, groupBy: 'lever_type' })

  // --- Candidates from presets
  let candidates = buildCandidates({
    weekKey,
    presets: PRESETS,
    gate: gateResult,
    deltas,
    primaryDrift: primary,
    daysSinceLeverRef: daysSince,
    corrNMax,
    experimentRowsMax
  })

  // --- Apply pattern priors (store multiplier)
candidates = candidates.map((o: any) => {
  const m1 = patternPrior({ patterns: byRef.items, groupBy: 'lever_ref', groupValue: o.leverRef, targetMetric: o.targetMetric })
  const m2 = patternPrior({ patterns: byType.items, groupBy: 'lever_type', groupValue: o.leverType, targetMetric: o.targetMetric })
  return { ...o, evidence: { ...(o.evidence ?? {}), patternPrior: m1 * m2 } }
})

const rankedRaw = scoreAndSort({ options: candidates as any, primaryDriftMetric: primary }).slice(0, 5)

const ranked = rankedRaw.map((o) => ({
  ...o,
  ui: formatOptionUI({ option: o, gate: gateResult })
}))
 ...
 const { data: upserted, error: e6 } = await supabase
  ...
  .upsert(
    ...
    {
      onConflict: 'user_id,week_key' // ✅ IMPORTANT
    }
  )
  .select('computed_at')
  .single()
  ...
  await supabase.from('insight_events').insert({
    user_id: uid, // ✅ not user.id
    kind: 'weekly_insight_computed',
    payload: { weekKey, gate: gateResult, driftPrimary: primary, corrNMax, experimentRowsMax, topOptionId: ranked[0]?.id }
  })
